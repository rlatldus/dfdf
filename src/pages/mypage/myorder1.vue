							...
							<div class="list" v-for="(item, index) in this.shippingList" :key="index">
								<div class="list_info">
									<div class="li_l">
										...
										<!-- <span class="status reserve" v-if="itemOrderInfo.SHIPPING_STATUS == 'P'">결제완료</span>
										<span class="status reserve" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'W'">준비중</span>
										<span class="status reserve" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'D'">배송중</span>
										<span class="status reserve" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'C'">완료</span>
										<span class="status reserve" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'Z'">구매확정</span> -->
									</div>
									<div class="li_r">
										<button type="button" :disabled="canclePending" class="btn order_cancel" v-if="item.isAllP" @click="orderCancelAll(item.paymentProductGroup, index)">구매취소</button>
										<router-link :to="'/gooduser/mypage/myorderdetail/'+item?.id" class="btn more">주문 상세보기</router-link>
									</div>
								</div>
								<div class="list_content" v-for="(itemOrderInfo, index2) in item.detail" :key="index2">
									...
									<!-- <div v-else>
										...
										<strong class="lc_tit" v-else-if="itemOrderInfo.PAYMENT_STATUS == 'W'">상품 준비중</strong>
										<strong class="lc_tit" v-else-if="itemOrderInfo.PAYMENT_STATUS == 'D'">배송중</strong>
										<strong class="lc_tit" v-else-if="itemOrderInfo.PAYMENT_STATUS == 'C'">배송 완료</strong>
									...
									">
										<div class="status01" v-if="item.shippingSpecialDayType == 'DayReservationDeliveryNormalDelivery' || item.shippingSpecialDayType == 'DayReservationDelivery'">
										...
										<div class="status01" v-if="item.shippingSpecialDayType == 'AnniversaryDelivery'">
											<span>기념일 예약 배송&nbsp;&nbsp;<em class="date mont">{{item.shippingDate}}</em></span>
										</div>
										<div class="status01" v-if="item.shippingSpecialDayType == 'HolidayDelivery'">
										...
										<div class="status02" v-if="
											...
											item.shippingSpecialDayType != 'DayReservationDelivery'
										">
											<a href="javascript:;" class="btn"
												@click="this.shipDateChangeOpen(itemOrderInfo.PRODUCT_ID, index2, item.shippingSpecialDayType, item, item.shippingDate)"
											>변경</a>
										</div>
										<div class="status02" v-if="itemOrderInfo.SHIPPING_LABEL_NUMBER">
											<div class="company">
												<span class="btn">{{itemOrderInfo.SHIPPING_LABEL_CARRIER}} {{itemOrderInfo.SHIPPING_LABEL_NUMBER}} <a :href="itemOrderInfo.SHIPPING_LABEL_LINK">배송현황 보러가기</a></span>
											</div>
										</div>
									</div>
									<ul class="product_list">
										<li>
												...
												<li>
													...
													<div class="pdt_info">
														<router-link :to='"/gooduser/goodslist/detail/"+itemOrderInfo.PRODUCT_ID'><strong class="name">
															{{itemOrderInfo.TEXT_NAME ? itemOrderInfo.TEXT_NAME : '상품명 세팅 오류'}}
														</strong></router-link>
											...
											<!-- 배송 전 케이스 -->
											<!-- <div v-if="itemOrderInfo.SHIPPING_STATUS">
												<strong class="lc_tit" v-if="itemOrderInfo.SHIPPING_STATUS == 'P'">결제완료</strong>
												<strong class="lc_tit" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'W'">상품 준비중</strong>
												<strong class="lc_tit" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'D'">배송중</strong>
												<strong class="lc_tit" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'C'">배송 완료</strong>
												<strong class="lc_tit" v-else-if="itemOrderInfo.SHIPPING_STATUS == 'Z'">구매확정</strong>
											</div>
											<div v-else> -->
											<!-- 배송중 케이스 -->
											<!-- <div class="btn_area">
												<button type="button" class="btn popup-open" data-popup="#popup-cancel">취소요청</button>
												<button type="button" class="btn popup-open" data-popup="#popup-goods">상품문의</button>
												<button type="button" class="btn popup-open" data-popup="#popup-call">전화문의</button>
											</div> -->
											<!-- 배송 완료 케이스 -->
											<div class="btn_area">
												<!-- 주문완료 -->
												<button type="button" :disabled="canclePending" v-if="itemOrderInfo.SHIPPING_STATUS == 'P'" class="btn" @click="orderCancel(item.paymentProductGroup, itemOrderInfo.PAYMENT_PRODUCT_ID)">구매취소</button>
												...
												v-if="
													...
													item.isReviewWritted == 'N'
												" class="btn popup-open" @click="this.reviewWrite(item.productReviewId, itemOrderInfo)">리뷰 작성</button>
		...
		<div class="popup-cont w01 on" id="popup-call" v-if="this.callQnaPop">
			...
			<div class="popup-body">
				<div class="call_wrap">
					...
					<p class="num mont">{{this.changeTel(callQnaNumber)}}</p>
					<dl class="call_info">
						<dt>상담가능시간</dt>
						<dd>
							{{callQnaTime}}
		...
		<div class="popup-cont w01" :class="{'on': reviewWriteData != null}" id="popup-review" v-if="reviewWriteData != null">
			...
			<div class="popup-body">
				<div class="goods_area">
					<div class="goods_img">
						<div class="img" :style="`background-image: url(${reviewWriteData.product.mainImage})`">
...
export default {
    ...
    methods: {
		...
		async getShippingList(){
			...
			let url = '/gooduser/api/v1/shipping_api/shipping_my_list';
			let setData = {
					'memberId': this.userId,
					'page': this.thisPage,
					'startDate': this.range?.start == null ? null : this.range?.start,
					'endDate': this.range?.end == null ? null : this.range?.end
				}
				console.log(setData);
            await axios.post(url,setData,{headers: {T: localStorage.getItem("T"), I: localStorage.getItem("I")}}
				...
				if(!this.shippingList){
					...
					this.shippingList.forEach((item, index) => {
						item.isAllP = true;
						this.shippingList[index].detail = JSON.parse(item.detail);
						this.shippingList[index].detail.forEach((item2, index2) => {
							if (item2.SHIPPING_STATUS != 'P') {
								item.isAllP = false;
							}
							//console.log(item2.MAIN_IMAGE_PATH.split('/'));
							this.shippingList[index].detail[index2].MAIN_IMAGE_PATH = item2.MAIN_IMAGE_PATH;
							this.shippingList[index].detail[index2].BAG_OPTION = JSON.parse(item2.BAG_OPTION);
							this.shippingList[index].detail[index2].CHOOSE_OPTION = JSON.parse(item2.CHOOSE_OPTION);
						});
					})
				}
		...
		async orderCancelAll (paymentProductGroup, shipIndex) {
			...
			this.shippingList[shipIndex].detail.forEach((item, index) => {
		...
		async reviewWrite (idx, prodInfo) {
			...
			await axios.post(url,setData,{headers: {T: localStorage.getItem("T"), I: localStorage.getItem("I")}}).then(res=>{
				...
				this.reviewWriteData.product = {
					mainImage: prodInfo.MAIN_IMAGE_URL,
		...
		async reviewWriteRequest () {
			...
			if(!this.reviewWriteStar){alert('별점을 등록해 주세요');}
