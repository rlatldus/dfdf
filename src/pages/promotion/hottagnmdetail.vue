<template>
    ...
    <div id="content" v-else>
		<!-- <div class="top-banner-promotion">
			<img v-for="(item, index) in this.eventList.pcImages" :key="index" :src="getPic(item)" :alt="item" class="for-pc">
			<img v-for="(item, index) in this.eventList.mobileImages" :key="index" :src="getPic(item)" :alt="item" class="for-m">
		</div> -->

		<div class="board-head for-event">
			<div class="w-set">
				<h2 class="post-title">{{this.eventList.name}}</h2>
				<p class="post-desc">{{this.eventList.startDate}} ~ {{this.eventList.endDate}}</p>
			</div>
		</div>

		<div class="w-set">
			<div v-html="this.eventList.content"></div>

			<div class="event-goods">
				<div class="goods-list grid-4 m-grid-2">
					<li class="goods-item" v-for="(item, index) in this.eventList.detail" :key="index">
						<div class="pic">
							<div class="like">
								<input
									type="checkbox"
							...
							<router-link :to="`/gooduser/goodslist/detail/${item.PRODUCT_ID}`">
								<img :src="item.MAIN_IMAGE_PATH" :alt="item.MAIN_IMAGE_PATH">
							</router-link>
						</div>
						<router-link :to="`/gooduser/goodslist/detail/${item.PRODUCT_ID}`" class="goods-info">
							<span class="goods-name">
								<strong class="brand" style="display: none;"></strong>
								<strong class="name">{{item.TEXT_NAME}}</strong>
							</span>
							<span class="goods-price">
								<span class="orig" v-if="item.VIP_SALE_PERCENT != '0.00' && item.NORMAL_SALE_PERCENT != '0.00'"><em class="num">{{threeCommaAdd(this.userGrade == 'VIP' ? item.VIP_NORMAL_PRICE : item.NORMAL_PRICE)}}</em>원</span>
								<span class="price-box">
									<span class="discount" v-if="item.VIP_SALE_PERCENT != '0.00' && item.NORMAL_SALE_PERCENT != '0.00'"><em class="num">
										{{Math.ceil(Number(this.userGrade == 'VIP' ? item.VIP_SALE_PERCENT : item.NORMAL_SALE_PERCENT))}}</em>%
									</span>
									<span class="price"><em class="num">{{threeCommaAdd(this.userGrade == 'VIP' ? item.VIP_SELLING_PRICE : item.NORMAL_SELLING_PRICE)}}</em>원</span>
								</span>
							</span>
						</router-link>
					</li>
...
export default {
    ...
    methods: {
		...
		async getEventDetail(){
			let url = `/gooduser/api/v1/promotion/exhibition/detail`
			let postData = {
			...
			await axios.post(url, postData,{headers: {T: localStorage.getItem("T"), I: localStorage.getItem("I")}}).then(res=>{
				...
				if (this.eventList.detail != null) {
					this.eventList.detail = JSON.parse(this.eventList.detail);
				}

				// if (this.eventList.list.content?.length > 0) {
				// 	let maxLength = this.eventList.list.content.length;
				// 	for(let i=0; i <= maxLength; i++) {
				// 		if (this.eventList.list.content[i] == null) {
				// 			this.eventList.list.content.splice(i ,1);
				// 			i = i-1;
