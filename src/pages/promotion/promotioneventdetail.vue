<template>
    ...
    <div id="content" v-else>
		...
		<div class="w-set">
			<div class="event-detail-content" v-html="this.eventList.content"></div>
...
export default {
    ...
    methods: {
		...
		async getEventDetail(){
			let url = `/gooduser/api/v1/promotion/event/detail`
			let postData = {
			...
			await axios.post(url, postData).then(res=>{
				...
				// if (this.eventList.detail?.length > 0) {
				// 	let maxLength = this.eventList.list.length;
				// 	for(let i=0; i <= maxLength; i++) {
				// 		if (this.eventList.list[i] == null) {
				// 			this.eventList.list.splice(i ,1);
				// 			i = i-1;
